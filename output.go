...
func OutputFiles(directory string, g *Generator, pkg string) error {
	structs := g.Structs
	// aliases := g.Aliases

	for _, name := range getOrderedStructNames(structs) {
		cs := structs[name]
		...
		filePath := fmt.Sprintf("%v/%v.go", directory, name)
		...
		fmt.Fprintln(writer, "// Code generated by schema-generate. DO NOT EDIT.")
		fmt.Fprintln(writer)
		...
		if cs.GenerateCode {
			emitMarshalCode(codeBuf, cs, imports)
			emitUnmarshalCode(codeBuf, cs, imports)
		}
		...
		fmt.Fprintln(writer, "")
		outputNameAndDescriptionComment(cs.Name, cs.Description, writer)
		fmt.Fprintf(writer, "type %s struct {\n", cs.Name)

		for _, fieldKey := range getOrderedFieldNames(cs.Fields) {
			f := cs.Fields[fieldKey]
			...
			if f.Description != "" {
				outputFieldDescriptionComment(f.Description, writer)
			}

			fmt.Fprintf(writer, "  %s %s `json:\"%s%s\"`\n", f.Name, f.Type, f.JSONName, omitempty)
		}
